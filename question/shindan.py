...
def sbtn_in(self):
    start_btn.config(fg="#FF5FF7")
def sbtn_out(self):
    start_btn.config(fg="#3271A1")
def mbtn_in(self):
    main_btn.config(fg="#3271A1")
def mbtn_out(self):
    main_btn.config(fg="grey")
# ------------------------------------------------------------------------------
# start
def start():
    global flg
...
start_btn.bind("<Enter>", sbtn_in)
start_btn.bind("<Leave>", sbtn_out)
main_btn = tk.Button(text="診断する", font=("System", 20), cursor="pointinghand",
                    fg="grey", bg="white", command=loading)
main_btn.bind("<Enter>", mbtn_in)
main_btn.bind("<Leave>", mbtn_out)
